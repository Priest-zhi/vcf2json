...
class Transform(object):
    ...
    def IoOperat_multi(self, tmpfile, mode, statisticArr, chunker):
        ...
        recordstring, infonum = self.chunker2string(chunker, fields, samples, mode)
    ...
    def vcf2json_multi2(self, filepath_vcf, filepath_json, md5, mode):
        ...
        for chunker in chunks:
            ...
            if index % processnum == 0:
                pool.map(partial(self.IoOperat_multi, tmpfile, mode, statisticArr), tmpchunks)
                # time.sleep(10)
                tmpchunks.clear()
        ...
        with open(statisticFile, mode='a') as fp:
            result = (filepath_vcf + '\t' + 'chrom: ' + '{0}' + '\t' + 'info: ' + '{1}' + '\n').format(statisticArr[0], statisticArr[1])
            fp.write(result)
...
if __name__ == "__main__":
    filepath = sys.argv[1]
    transform = Transform()
    transform.dotranform(filepath, mode='MergeAll')
